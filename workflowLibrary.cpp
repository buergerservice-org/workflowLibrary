...
namespace workflowLibrary
{
    ...
    std::string workflow::getkeypad()
    {
        ...
        try
        {
            ...
            {
                std::cout << "AusweisApp2-version equal or greater 1.22.*, ok wie continue." << std::endl;
            }
    ...
    std::string workflow::startworkflow(std::string PINstring)
    {
        ...
        try
        {
            ...
            ws.set_option(websocket::stream_base::decorator(
                [](websocket::request_type& req)
                {
                    req.set(http::field::user_agent,
                        //std::string(BOOST_BEAST_VERSION_STRING) +
                        //" websocket-client-coro");
                        "buergerservice.org e.V. workflowLibary");
                }));

            //typedef boost::asio::detail::socket_option::integer<SOL_SOCKET, SO_RCVTIMEO> rcv_timeout_option;
            //boost::beast::get_lowest_layer(ws).set_option(rcv_timeout_option{ 1000 });
            ...
            //boost::beast::get_lowest_layer(ws).set_option(rcv_timeout_option{ 7000 });
